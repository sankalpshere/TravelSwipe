...
public class ScreenSlideActivity extends FragmentActivity {
	...
	private HashMap<String, TravelMetadata> data;
	private HashMap<String, Integer> cityScores;
	...
	private void startGestureTest() {
		...
		if (mTestMode == MODE_HAND_PRIMITIVE) {
	...
	/*private void displayData(SgestureHand.Info info) {
		...
		if (info.getType() == Sgesture.TYPE_HAND_PRIMITIVE) {
			...
			System.out.println("Direction " + direction + " - " + mPager.getCurrentItem());

			int index = mPager.getCurrentItem();
			if (index >= 0 && index <= NUM_PAGES) {
				System.out.println("index before  swipe " + index);
				if (index <= NUM_PAGES && index > 0
						&& Direction.LEFT.equals(direction)) {
					mPager.setCurrentItem(mPager.getCurrentItem() - 1);
					System.out.println("index after left swipe " + index);
				}
				if (index >= 0 && index != NUM_PAGES - 1
						&& Direction.RIGHT.equals(direction)) {
					mPager.setCurrentItem(mPager.getCurrentItem() + 1);
					System.out.println("index after right swipe " + index);
				}
